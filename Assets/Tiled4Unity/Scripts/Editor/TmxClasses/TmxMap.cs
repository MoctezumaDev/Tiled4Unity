...
namespace Tiled4Unity
{
    public partial class TmxMap : TmxHasProperties
    {
        ...
        {
            return String.Format("{0}.tiled4unity.xml", this.Name);
        }
        ...
        public Vector2 GetMapPositionAt(int x, int y)
        {
            return TmxMath.TileCornerInScreenCoordinates(this, x, y);
        }

        public Vector2 GetMapPositionAt(int x, int y, TmxTile tile)
        {
            Vector2 point = GetMapPositionAt(x, y);
        ...
        {
            Console.WriteLine("Loading Object Type Xml file: '{0}'", xmlPath);
            ...
            {
                Console.WriteLine("Object Type Xml file was not found: {0}", xmlPath);
                this.ObjectTypes = new TmxObjectTypes();
            }
            catch (Exception e)
            {
                Console.WriteLine("Error parsing Object Type Xml file: {0}\n{1}", xmlPath, e.Message);
                this.ObjectTypes = new TmxObjectTypes();
            }

            Console.WriteLine("Tiled Object Type count = {0}", this.ObjectTypes.TmxObjectTypeMapping.Count());
        }

        public void ClearObjectTypeXml()
        {
            Console.WriteLine("Removing Object Types from map.");
            this.ObjectTypes = new TmxObjectTypes();
        }
