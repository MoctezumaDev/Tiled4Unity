...
namespace Tiled4Unity
{
    ...
    partial class TiledMapExporter
    {
        ...
        private void AddPolygonCollider2DElements_Convex(ClipperLib.PolyTree solution, List<XElement> xmlList)
        {
            ...
            {
                string data = String.Join(" ", pointfArray.Select(pt => String.Format("{0},{1}", pt.x * Settings.Scale, pt.y * Settings.Scale)).ToArray<string>());
        ...
        private void AddPolygonCollider2DElements_Complex(ClipperLib.PolyTree solution, List<XElement> xmlList)
        {
            ...
            {
                string data = String.Join(" ", path.Select(pt => String.Format("{0},{1}", pt.X * Settings.Scale, pt.Y * Settings.Scale)).ToArray<string>());
                XElement pathElement = new XElement("Path", data);
        ...
        private void AddEdgeCollider2DElements(ClipperPolygons lines, List<XElement> xmlList)
        {
            ...
            {
                string data = String.Join(" ", points.Select(pt => String.Format("{0},{1}", pt.X * Settings.Scale, pt.Y * Settings.Scale)).ToArray<string>());
