...
namespace Tiled4Unity
{
    ...
    public class LayerClipper
    {
        ...
        public static ClipperLib.PolyTree ExecuteClipper(TmxMap tmxMap, TmxLayer tmxLayer, TransformPointFunc xfFunc, ProgressFunc progFunc)
        {
            ...
            var polygonGroups = from y in Enumerable.Range(0, tmxLayer.Height)
                                ...
                                group new
                                {
                                    ...
                                    TileCenter = new Vector2(tile.TileSize.Width * 0.5f, tile.TileSize.Height * 0.5f),
                                }
                                by new Tuple<int,int>(groupX, groupY);
            ...
            foreach (var polyGroup in polygonGroups)
            {
                ...
                foreach (var poly in polyGroup)
                {
                    ...
                    Vector2 offset = poly.TmxObjectInterface.Position;
                    Vector2[] transformedPoints = poly.HasPointsInterface.Points.Select(pt => (pt+offset)).ToArray();
        ...
        public static IEnumerable<Vector2[]> SolutionPolygons_Complex(ClipperLib.PolyTree solution)
        {
            foreach (var points in ClipperLib.Clipper.ClosedPathsFromPolyTree(solution))
            {
                var pointfs = points.Select(pt => new Vector2(pt.X, pt.Y));
        ...
        public static IEnumerable<Vector2[]> SolutionPolygons_Simple(ClipperLib.PolyTree solution)
        {
            ...
            List<Vector2[]> triangles = triangulation.Triangulate(solution);
#if T2U_TRIANGLES
            ...
            List<Vector2[]> polygons = composition.Compose(triangles);
