    ...
    public class PolygonEdge
    {
        ...
        public bool CanMergePolygons()
        {
            ...
            // Can merge point P of the Major/CCW partner?
            {
                ...
                // B = CWW[P]
                // C = CW[P + 1]
                Vector2 A = this.MajorPartner.PrevPoint(this.MajorPartner_pIndex);
                Vector2 B = this.MajorPartner.Points[this.MajorPartner_pIndex];
                Vector2 C = this.MinorPartner.NextPoint(this.MinorPartner_pIndex);
            ...
            // Can merge point Q of Major/CCW partner?
            {
                ...
                // B = CWW[Q]
                // C = CW[Q-1]
                Vector2 A = this.MajorPartner.NextPoint(this.MajorPartner_qIndex);
                Vector2 B = this.MajorPartner.Points[this.MajorPartner_qIndex];
                Vector2 C = this.MinorPartner.PrevPoint(this.MinorPartner_qIndex);
                float cross = Geometry.Math.Cross(A, B, C);
                if (cross < 0)
                    return false;
            }
